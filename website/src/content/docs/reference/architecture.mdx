---
title: Architecture
description: Runtime pipeline, lifecycle, storage boundaries, and package design invariants.
sidebar:
  order: 6
---

This document describes how the published `@rikology/adonisjs-periscope` package is put together.
For usage and configuration, see [Get started](/adonisjs-periscope/get-started); for contribution workflow, invariants,
and verification commands, see the
[contribution guide](https://github.com/rikoriswandha/adonisjs-periscope/blob/main/CONTRIBUTING.md).
...
See the [HTTP API reference](/adonisjs-periscope/reference/http-api) for endpoint schemas and the stability contract.
