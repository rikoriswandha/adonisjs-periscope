...
export async function GET({ props }: { props: SlugProps }) {
  ...
  const body = [
    ...
    ...(socialImage ? [`image: ${JSON.stringify(absoluteSiteUrl(socialImage, config.site))}`] : []),
    ...(version ? [`version: ${JSON.stringify(version)}`] : []),
    '---',
    '',
    '> Documentation Index',
    `> Fetch the complete documentation index at: ${absoluteSiteUrl('/llms.txt', config.site)}`,
    ...
    `Source: ${absoluteSiteUrl(sourceUrl ?? markdownUrl, config.site)}`,
    '',
  ].join('\n')
