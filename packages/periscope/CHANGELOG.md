...
- 6751d72: Production-readiness hardening across security, durability, and packaging:
  ...
  - **Durability**: recorder flushes no longer drop batches on failed saves (bounded retention and retry); database driver write backlog is bounded with oldest-drop backpressure; shutdown drains watcher-owned in-flight flushes; `maxEntries` is enforced as a hard ceiling with an O(1) in-process count check; collision-safe composite `(sequence, uuid)` ordering across stores; SQLite uses WAL (bounded by `journal_size_limit`) with a 100 ms busy timeout, creates database files with `0600` permissions, and self-heals torn `-wal`/`-shm` sidecars left by an ungraceful kill instead of failing every subsequent boot.
