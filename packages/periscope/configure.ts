...
const PROVIDER_PATH = '@rikology/adonisjs-periscope/provider'
const COMMANDS_PATH = '@rikology/adonisjs-periscope/commands'
...
function ensureRuntimeReporterImport(file: SourceFile) {
  ...
  if (valueReporterImport) {
    ...
    file.addImportDeclaration({
      moduleSpecifier: '@rikology/adonisjs-periscope/exception_reporter',
...
function printChecklist(
  ...
  command.logger.log(
    `${complete(outcomes.middleware) ? '[x]' : '[ ]'} Keep @rikology/adonisjs-periscope/middleware/request_watcher first in server.use([...])`
  )
  command.logger.log(
